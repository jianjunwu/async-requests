...
from async_requests.tools import retry
...
class AsyncRequest(object):
    ...
    def send(self, **kwargs):
        ...
        try:
            # self.response = self.session.request(self.method,
            #                                      self.url, **merged_kwargs)
            self.response = self._send(**merged_kwargs)
        except Exception as e:
            self.exception = e
            self.traceback = traceback.format_exc()
        finally:
...
def collect_async_results(requests, stream=False, size=None, exception_handler=None, gtimeout=None):
